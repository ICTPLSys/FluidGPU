...
max_new_tokens=16
prompt="${PROMPT:-Describe robustness under slow interconnect conditions for GPT-oss.}"
out_dir=""
extra_args=()
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --allow-transformers-mismatch|--strict-transformers-version)
      extra_args+=("$1"); shift ;;
    *) echo "run_fig12a_slow_network_fluidgpu.sh: unknown argument $1" >&2; exit 2 ;;
  esac
done

if [[ -z "${out_dir}" ]]; then
  ts="$(date -u +%Y%m%dT%H%M%SZ)"
  out_dir="${FLUIDGPU_RUN_OUTPUT_DIR:-artifacts/logs/fig12a/${ts}}"
fi
mkdir -p "${out_dir}"

printf '%s\n' "${comm_bw_gbps}" > "${out_dir}/comm_bw_gbps.txt"
printf '%s\n' "${mode}" > "${out_dir}/mode.txt"
...
if [[ -n "${comm_bw_gbps}" ]]; then
  export FLUIDGPU_COMM_BW_GBPS="${comm_bw_gbps}"
fi
export FLUIDGPU_SLOW_NETWORK_MODE="${mode}"
exec "${cmd[@]}"
