...
max_new_tokens=16
prompt="${PROMPT:-Describe online monitor sensitivity for GPT-oss queueing control.}"
out_dir=""
extra_args=()
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --allow-transformers-mismatch|--strict-transformers-version)
      extra_args+=("$1"); shift ;;
    *) echo "run_fig11_monitor_sensitivity_fluidgpu.sh: unknown argument $1" >&2; exit 2 ;;
  esac
done

if [[ -z "${out_dir}" ]]; then
  ts="$(date -u +%Y%m%dT%H%M%SZ)"
  out_dir="${FLUIDGPU_RUN_OUTPUT_DIR:-artifacts/logs/fig11/${ts}}"
fi
mkdir -p "${out_dir}"

printf '%s\n' "${monitor_window_ms}" > "${out_dir}/monitor_window_ms.txt"
printf '%s\n' "${monitor_beta}" > "${out_dir}/monitor_beta.txt"
...
export FLUIDGPU_ENABLE_ONLINE_MONITOR=1
export FLUIDGPU_MONITOR_WINDOW_MS="${monitor_window_ms}"
export FLUIDGPU_MONITOR_BETA="${monitor_beta}"
exec "${cmd[@]}"
